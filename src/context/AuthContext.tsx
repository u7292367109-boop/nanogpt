...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function fetchProfile(userId: string) {
    ...
    if (data) {
      ...
      // Auto-create profile for users who registered before this was in place
      const newProfile = {
        id: userId,
        username: 'User' + generateUID().slice(0, 6),
        uid: generateUID(),
  ...
  async function fetchAssets(userId: string) {
    const { data } = await supabase.from('assets').select('*').eq('user_id', userId).maybeSingle()
    if (data) {
      setAssets(data)
    } else {
      // Auto-create assets row for users who are missing it
      const newAssets = {
        user_id: userId,
        task_balance: 0,
        vault_balance: 0,
      ...
      const { data: created } = await supabase.from('assets').insert(newAssets).select().single()
      if (created) setAssets(created)
    }
  }

  async function ensureDevice(userId: string) {
    const { data } = await supabase.from('devices').select('*').eq('user_id', userId).maybeSingle()
    if (!data) {
      ...
      const di = detectDeviceInfo()
      const stale = data.os === 'NanoGPT OS' || data.os === 'Unknown OS' || data.model === 'Computing Node'
      if (stale) {
